...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_proportion", type=int, default=0.9)
    parser.add_argument("--gpu_devices", nargs="+", default=[0])
    args = parser.parse_args()

    logger_name = args.output + ".log"
    ...
    for idx in misclassified_indices:
        image = all_images[idx][0][0]
        pred = all_predictions[idx]
        truth = all_truths[idx]
        filename = all_filenames[idx]

        json_filename = filename.split('.')[0] + '.json'
        json_filename = os.path.join(args.error_analysis_dir, json_filename)
